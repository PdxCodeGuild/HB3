...
if number_input >= 0 and number_input <= 19:
    ...
    if tens_digit == 0 and ones_digit == 0:
        print(f'\n\t{number_input} spelled out is: {hundreds[hundred_digit]}')
    elif number_input%100 in ones:
        hundred_word = hundreds[hundred_digit]
        print(f'\n\t{number_input} spelled out is: {hundred_word} & {ones[number_input%100]}')
    else:
        hundred_word = hundreds[hundred_digit]
        tens_word = tens[tens_digit]
        ones_word = ones[ones_digit] 
        if ones_digit == 0:
            print(f'\n\t{number_input} spelled out is: {hundred_word} & {tens_word}')
        else:
