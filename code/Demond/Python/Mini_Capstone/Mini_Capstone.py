...
import re
import os
#I have now ensured that the mail, email, and content class are imported
...
set_price = float(input('Enter the price you would like to be notified for: '))
...
def Email_Notification():
    ...
    response = sendgrid.SendGridAPIClient('I cannot share my API key for authorization here or my account gets locked')
    #this is where you can input your approved and authenticated domain
    #The from, to, and content variables are all stored within an object named "personalization" from the API
    ...
    content = Content("text/plain", "Your Stock Has Reached Its Point!")
    #this mail function wil actually construct the mail
    mail = Mail(from_email, to_email, subject, content)
    #this will convert the mail into a json
    mail_json = mail.get()
    # this sends an HTTP POST request to /mail/send it specifically checks if my response matches my json variable
    x = response.client.mail.send.post(request_body=mail_json)
...
if int(set_price) >= int(user_stock_price):
        Email_Notification()
        # break